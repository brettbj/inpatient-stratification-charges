...
spark_df = spark_df.withColumn('losLabel', (f.when(merged_pats['LOS']>6) | (f.when(merged_pats['disc_status']==20), 1).otherwise(0))
spark_df = spark_df.withColumn('costLabel', merged_pats['pat_cost'])
spark_df = spark_df.withColumn('readmitLabel', (f.when(merged_pats['readmit_time']<31) | (f.when(merged_pats['disc_status']==20), 1).otherwise(0))
